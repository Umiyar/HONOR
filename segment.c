...
static void new_curseg(struct f2fs_sb_info *sbi, int type, bool new_sec)
{
	...
	if (seg_type == CURSEG_WARM_DATA || seg_type == CURSEG_COLD_DATA || seg_type == CURSEG_HOT_DATA)
		dir = ALLOC_RIGHT;
		...
		struct f2fs_io_info *fio)
{
	...
	*new_blkaddr = NEXT_FREE_BLKADDR(sbi, curseg);
	// printk("new_blkaddr:%u",*new_blkaddr);
					...
					struct f2fs_io_info *fio)
{
	...
	struct f2fs_summary sum;
	// block_t blkaddr_distance = dn->data_blkaddr;
	f2fs_bug_on(sbi, dn->data_blkaddr == NULL_ADDR);
	set_summary(&sum, dn->nid, dn->ofs_in_node, fio->version);
	do_write_page(&sum, fio);
	// blkaddr_distance = fio->new_blkaddr-blkaddr_distance;
	// printk("fio->new_blkaddr:%u,fio->old_blkaddr:%u,dn->data_blkaddr:%u,dn'blkaddr_distance:%u ",fio->new_blkaddr,fio->old_blkaddr,dn->data_blkaddr,blkaddr_distance);
